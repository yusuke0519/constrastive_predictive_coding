...
cuda="OMP_NUM_THREADS=4"

zikkou() {
    eval $cuda "python run_sacred.py -x T" $base $1
    eval $cuda "python run_sacred.py -x T" $base $1 'method.sampler_mode="diff"'
    eval $cuda "python run_sacred.py -x T" $base $1 'method.sampler_mode="same"'
}
# context size
